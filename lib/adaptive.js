/**
 * Copyright contributors to the IBM Security Verify Adaptive Proxy SDK for JavaScript project.
 */
        ...
        enrolledFactors = enrolledFactors.filter((enrollment) =>
          assessment.allowedFactors.includes(enrollment.type) || (
            enrollment.type === 'signature' && (
              // allowedFactors specifies only signatures (old)
              assessment.allowedFactors.includes('signatures') ||
              // allowedFactors includes signature and subType (new)
              assessment.allowedFactors.includes('signatures_' + enrollment.subType)
            )
          )
        );
      }
